...
class ConnectivityService {
  ...
  static const Duration _internetCheckTimeout = Duration(seconds: 3);
  ...
  void _handleConnectivityChange(List<ConnectivityResult> results) async {
    final hasNetworkConnection = _hasActiveConnection(results);
    ...
    if (!hasNetworkConnection) {
      ...
      if (wasConnected != _isConnected) {
        debugPrint('Connectivity changed: Offline (no network)');
  ...
  Future<bool> _checkInternetAccess() async {
    try {
      ...
      for (final host in hosts) {
        try {
          final result =
              await InternetAddress.lookup(host).timeout(_internetCheckTimeout);
